% Tres missionarios e tres canibais, que se encontram na margem
% esquerda de um rio, querem atravessar a margem direita. O barco
% existente transporta, no máximo, duas pessoas. Determine as operações
% a realizar, sabendo que o número de canibais não pode ser superior ao
% número de missionários em qualquer margem do rio.
...
atravessa(Ea, Ef,[Ea|R], Eants) :- seguinte(Ea,Eseg),
                                   seguro(Eseg),
                                   not(member(Eseg,Eants)),
                                   atravessa(Eseg, Ef, R, [Eseg|Eants]).

miss_can :- inicial(Ei), final(Ef), 
